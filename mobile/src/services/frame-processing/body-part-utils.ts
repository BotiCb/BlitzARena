...
import { BODY_PART, Keypoints, KEYPOINTS, Point } from '~/utils/types/detection-types';
...
function isChestHit(keypoints: Keypoints): boolean {
  ...
  const isLeftPartHit =
    ...
    (keypoints[KEYPOINTS.LEFT_SHOULDER]?.coord && keypoints[KEYPOINTS.LEFT_HIP]?.coord
      ? distance(keypoints[KEYPOINTS.LEFT_SHOULDER].coord, keypoints[KEYPOINTS.LEFT_HIP].coord) *
        BODY_PART_DETECTIONS_CONSTANTS.CHEST_SIDES_PADDING
      : Infinity);
  ...
  const isRightPartHit =
    ...
    (keypoints[KEYPOINTS.RIGHT_SHOULDER]?.coord && keypoints[KEYPOINTS.RIGHT_HIP]?.coord
      ? distance(keypoints[KEYPOINTS.RIGHT_SHOULDER].coord, keypoints[KEYPOINTS.RIGHT_HIP].coord) *
        BODY_PART_DETECTIONS_CONSTANTS.CHEST_SIDES_PADDING
      : Infinity);
  ...
  const isTopPartHit =
    ...
    (keypoints[KEYPOINTS.LEFT_SHOULDER]?.coord && keypoints[KEYPOINTS.RIGHT_SHOULDER]?.coord
      ? distance(
          keypoints[KEYPOINTS.LEFT_SHOULDER].coord,
          keypoints[KEYPOINTS.RIGHT_SHOULDER].coord
        ) * BODY_PART_DETECTIONS_CONSTANTS.CHEST_TOP_PADDING
      : Infinity);
...
function isLegHit(keypoints: Keypoints): boolean {
  ...
  const isLeftUpperLegHit =
    ...
    (keypoints[KEYPOINTS.LEFT_HIP]?.coord && keypoints[KEYPOINTS.LEFT_KNEE]?.coord
      ? distance(keypoints[KEYPOINTS.LEFT_HIP].coord, keypoints[KEYPOINTS.LEFT_KNEE].coord) *
        BODY_PART_DETECTIONS_CONSTANTS.UPPER_LEG_PADDING
      : Infinity);
  ...
  const isRightUpperLegHit =
    ...
    (keypoints[KEYPOINTS.RIGHT_HIP]?.coord && keypoints[KEYPOINTS.RIGHT_KNEE]?.coord
      ? distance(keypoints[KEYPOINTS.RIGHT_HIP].coord, keypoints[KEYPOINTS.RIGHT_KNEE].coord) *
        BODY_PART_DETECTIONS_CONSTANTS.UPPER_LEG_PADDING
      : Infinity);
  ...
  const isLeftLowerLegHit =
    ...
    (keypoints[KEYPOINTS.LEFT_KNEE]?.coord && keypoints[KEYPOINTS.LEFT_ANKLE]?.coord
      ? distance(keypoints[KEYPOINTS.LEFT_KNEE].coord, keypoints[KEYPOINTS.LEFT_ANKLE].coord) *
        BODY_PART_DETECTIONS_CONSTANTS.LOWER_LEG_PADDING
      : Infinity);
  ...
  const isRightLowerLegHit =
    ...
    (keypoints[KEYPOINTS.RIGHT_KNEE]?.coord && keypoints[KEYPOINTS.RIGHT_ANKLE]?.coord
      ? distance(keypoints[KEYPOINTS.RIGHT_KNEE].coord, keypoints[KEYPOINTS.RIGHT_ANKLE].coord) *
        BODY_PART_DETECTIONS_CONSTANTS.LOWER_LEG_PADDING
      : Infinity);
...
function isHandHit(keypoints: Keypoints): boolean {
  ...
  const isLeftUpperArmHit =
    ...
    (keypoints[KEYPOINTS.LEFT_SHOULDER]?.coord && keypoints[KEYPOINTS.LEFT_ELBOW]?.coord
      ? distance(keypoints[KEYPOINTS.LEFT_SHOULDER].coord, keypoints[KEYPOINTS.LEFT_ELBOW].coord) *
        BODY_PART_DETECTIONS_CONSTANTS.UPPER_ARM_PADDING
      : Infinity);
  ...
  const isLeftLowerArmHit =
    ...
    (keypoints[KEYPOINTS.LEFT_ELBOW]?.coord && keypoints[KEYPOINTS.LEFT_WRIST]?.coord
      ? distance(keypoints[KEYPOINTS.LEFT_ELBOW].coord, keypoints[KEYPOINTS.LEFT_WRIST].coord) *
        BODY_PART_DETECTIONS_CONSTANTS.LOWER_ARM_PADDING
      : Infinity);
  ...
  const isRightLowerArmHit =
    ...
    (keypoints[KEYPOINTS.RIGHT_ELBOW]?.coord && keypoints[KEYPOINTS.RIGHT_WRIST]?.coord
      ? distance(keypoints[KEYPOINTS.RIGHT_ELBOW].coord, keypoints[KEYPOINTS.RIGHT_WRIST].coord) *
        BODY_PART_DETECTIONS_CONSTANTS.LOWER_ARM_PADDING
      : Infinity);
...
function isHeadHit(keypoints: Keypoints): boolean {
  ...
  if (keypoints[KEYPOINTS.NOSE]?.coord && keypoints[KEYPOINTS.LEFT_SHOULDER]?.coord) {
    const noseToShoulderDistance = distance(
      keypoints[KEYPOINTS.NOSE].coord,
      keypoints[KEYPOINTS.LEFT_SHOULDER].coord
    );
    const headSize = noseToShoulderDistance;
    const isHeadHit =
      distance(keypoints[KEYPOINTS.NOSE].coord, BODY_PART_DETECTIONS_CONSTANTS.CENTER_POINT) <
      headSize;
    return isHeadHit;
  }
...
export function getHitBodyPartFromKeypoints(
  ...
  if (
    scopeCoordinates.x < 0 ||
    scopeCoordinates.y < 0 ||
    scopeCoordinates.x > 1 ||
    scopeCoordinates.y > 1
  ) {
